...
std_msgs::UInt64 psd_val_msg;
ros::Publisher psd_val_pub("psd_val", &psd_val_msg);
...
void loop()
{
  ...
  if ((millis()-tTime[4]) >= (1000 / PSD_PUBLISH_PERIOD))
  {
    readSensorPSD();
    tTime[4] = millis();
  }
...
void readSensorPSD()
{
  int psdValue = analogRead(A0);

  psd_val_msg.data = psdValue;

  psd_val_pub.publish(&psd_val_msg);
}
